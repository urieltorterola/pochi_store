...
// Las funciones de persistencia ya son correctas.
...
function addToCart(mangaId) {
    const cart = getCart();
    // Corregido: Usa mangasData para encontrar el producto, no mangasData2 (que es solo para el carrusel)
    // Se asume que el ID del carrito proviene de mangasData
    const manga = mangasData.find(m => m.id === mangaId) || mangasData2.find(m => m.id === mangaId);
    if (!manga) return;
    ...
    if (existingItem) {
        ...
        // Al agregar, calculamos el precio final para que sea consistente
        const finalPrice = manga.offer ? (manga.price * (1 - manga.discount)) : manga.price;
        cart.push({ ...manga, quantity: 1, finalPrice: finalPrice });
    }
...
function updateCartDisplay() {
    ...
    if (cart.length === 0) {
        // CORRECCIÓN: Estilo de mensaje de carrito vacío
        cartList.innerHTML = '<p class="empty-cart-message" style="text-align: center; color: gray; padding: 10px;">El carrito está vacío.</p>';
    } else {
        cart.forEach(item => {
            // CORRECCIÓN: Calcular precio final correctamente si no fue guardado al agregar
            const finalPrice = item.offer 
                ? (item.price * (1 - item.discount)).toFixed(2) 
                : item.price.toFixed(2);
                
            const itemTotal = (parseFloat(finalPrice) * item.quantity).toFixed(2); // Calculamos el total por ítem

            const li = document.createElement('li');
            // CORRECCIÓN: Mostramos la cantidad (xN) y el total por ítem
            li.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span>${item.title} (x${item.quantity})</span>
                    <span>$${itemTotal}</span>
                </div>
                <button class="boton" style="margin-top: 5px; font-size: 0.8em;" onclick="removeFromCart(${item.id})">Quitar</button>
            `;
...
// ... (código existente para carrusel) ...
